...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask=None):
        ...
        #print(f"Original q shape: {q.shape}, k shape: {k.shape}, v shape: {v.shape}")
        ...
        # 假设 self.fc 的输入维度是 128，你需要确保 q 的形状匹配
        input_dim = 128

        # 在调用 self.fc 之前打印 q 的形状
        #print(f"q shape before fc: {q.shape}")
        if q.size(-1) != input_dim:
